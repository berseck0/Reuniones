...
if ($op == 1) {
    ...
    {  
        ...
        {
            $mysqli =$this->conexion();
            $sql = "SELECT id_etiqueta,nombre FROM jb_etiquetas where nombre=? and id_usuario=?;";
            $lista=array();
...
if($op == 3)
{
    ...
    {  
        ...
        function usuariobusqueda($plaza)
        {
            ...
            $mysqli = $this->conexion();
            $sql = "SELECT jb_empleado.NOMBRES,jb_empleado.ID_EMPLEADO, jb_plaza.NOMBRE_PLAZA FROM jb_empleado, jb_plaza where jb_plaza.id=jb_empleado.ID_EMPLEADO and jb_plaza.NOMBRE_PLAZA=?;";
                $lista=array();
              
                if ($data=$mysqli->prepare($sql)) {
                    $data->bind_param("s",$plaza);
                     if($data->execute())
                     {
        ...
        {
            $mysqli = $this->conexion();
            $sql = "SELECT NOMBRES,ID_EMPLEADO FROM jb_empleado WHERE NOMBRES LIKE '%$nom%' limit 5";
